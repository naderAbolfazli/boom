...
class VideoMessage(DocumentMessage):
    ...
    def get_json_object(self):
        ...
        data["ext"] = {
            "$type": DocumentType.video_document,
    ...
    def load_from_json(cls, json):
        ...
        if (not file_id) or (not access_hash) or (name is None) or (file_size is None) \
                or (mime_type is None) or (width is None) or (height is None) or (ext_width is None) \
                or (ext_height is None) or (thumb_txt is None) or (duration is None):
        ...
        return cls(file_id=file_id, access_hash=access_hash, name=name, file_size=file_size,
                   mime_type=mime_type, thumb=thumb_txt, duration=duration, width=width, height=height,
                   file_storage_version=file_storage_version, caption_text=caption_message, checksum=checksum,
                   algorithm=algorithm)
